...
const rows = computed<Row[]>(() => {
  const list: Row[] = []
  for (const a of mesh.agents) {
    if (a.projectId !== network.projectId) continue
    for (const i of a.interfaces) {
      const n = mesh.networkById(i.networkId)
      list.push({
        ifaceId: i.id,
        agentName: a.name,
        online: a.status === 'online' && a.enabled,
        ifaceName: i.name,
        tunnelIP: i.tunnelIP,
        networkName: n?.name ?? i.networkId,
        cross: i.networkId !== network.id,
      })
    }
  }
  return list
...
const filtered = computed(() => {
  ...
  return rows.value.filter((r) => `${r.agentName} ${r.ifaceName} ${r.tunnelIP} ${r.networkName}`.toLowerCase().includes(kw))
})

const selectable = computed(() => rows.value.filter((r) => !r.cross))

function toggle(id: string) {
  const r = rows.value.find((x) => x.ifaceId === id)
  if (!r || r.cross) return
...
function selectAll() {
  selected.value = new Set(selectable.value.map((r) => r.ifaceId))
}
...
async function save() {
  // 将选中的真实节点两两提交为自定义 Peer
  const ids = selectable.value.filter((r) => selected.value.has(r.ifaceId)).map((r) => r.ifaceId)
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/70 p-4 backdrop-blur-sm" @click.self="emit('close')">
    <div class="panel flex max-h-[85vh] w-full max-w-2xl flex-col overflow-hidden">
      <div class="border-b border-ink-700 px-6 py-4">
        ...
        <p class="mt-0.5 text-xs text-slate-500">手动选择需要互联的接口；跨网络节点不可选，避免生成无效配置</p>
      </div>
      ...
      <div class="flex items-center gap-3 border-b border-ink-700 px-6 py-3">
        <div class="relative flex-1">
          ...
          <input v-model="keyword" class="input pl-9 !py-2" placeholder="搜索 Agent / 接口 / 隧道 IP…" />
        </div>
        ...
        <div class="space-y-1.5">
          <label
            ...
            :class="[
              ...
              selected.has(r.ifaceId) && !r.cross ? 'bg-emerald-500/10 ring-emerald-500/40' : 'ring-ink-700',
            ]"
          >
            <input type="checkbox" class="h-4 w-4 rounded border-ink-600 bg-ink-800 accent-emerald-500" :checked="selected.has(r.ifaceId)" :disabled="r.cross" @change="toggle(r.ifaceId)" />
            <span class="h-2 w-2 shrink-0 rounded-full" :class="r.online ? 'bg-emerald-400' : 'bg-slate-600'"></span>
            <div class="min-w-0 flex-1">
              <p class="text-sm text-slate-200">{{ r.agentName }} <span class="text-slate-500">/</span> <span class="font-mono text-xs">{{ r.ifaceName }}</span></p>
              <p class="font-mono text-[11px] text-slate-500">{{ r.tunnelIP }}</p>
            </div>
      ...
      <div class="flex items-center justify-between border-t border-ink-700 px-6 py-4">
        <p class="text-xs text-slate-500">已选 <span class="font-semibold text-emerald-300">{{ selected.size }}</span> 个接口</p>
