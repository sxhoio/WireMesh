...
import { fmtHandshake, shortKey } from '../utils/format'
  ...
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/70 p-4 backdrop-blur-sm" @click.self="emit('close')">
    <div class="panel flex max-h-[85vh] w-full max-w-3xl flex-col overflow-hidden">
      <div class="flex items-center justify-between border-b border-ink-700 px-6 py-4">
        <div>
          <h2 class="text-base font-semibold text-white">临时 Peer</h2>
          <p class="mt-0.5 text-xs text-slate-500">仅展示后端返回的临时 Peer；当前 WireMesh 后端尚未提供对应查询和管理 API</p>
        </div>
      ...
      <div class="min-h-0 flex-1 overflow-x-auto p-6">
        <table class="w-full min-w-[640px] text-left text-sm">
          <thead><tr class="border-b border-ink-700 text-xs text-slate-500"><th class="py-2.5 pr-4 font-medium">公钥</th><th class="py-2.5 pr-4 font-medium">Endpoint</th><th class="py-2.5 pr-4 font-medium">位置</th><th class="py-2.5 pr-4 font-medium">AllowedIPs</th><th class="py-2.5 pr-4 font-medium">来源接口</th><th class="py-2.5 font-medium">握手</th></tr></thead>
          <tbody>
            ...
            <tr v-if="!mesh.scopedTempPeers.length"><td colspan="6" class="py-12 text-center"><p class="text-sm text-slate-400">没有临时 Peer 数据</p><p class="mt-1 text-xs text-slate-600">页面不会生成或保存临时 Peer 本地记录。</p></td></tr>
          </tbody>
        </table>
      </div>
    </div>
