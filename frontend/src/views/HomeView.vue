    ...
    <div class="grid grid-cols-1 gap-5 2xl:min-h-0 2xl:flex-1 2xl:grid-cols-4">
      ...
      <div class="flex flex-col gap-5 2xl:min-h-0 2xl:col-span-3">
        ...
        <div class="grid shrink-0 grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          <!-- 实时动态 -->
          <div class="panel flex min-h-0 flex-col p-4">
            <h3 class="mb-3 text-sm font-semibold text-white">最近异常 / 实时动态</h3>
            <div class="min-h-0 max-h-64 flex-1 space-y-2.5 overflow-y-auto p-2">
              <div v-for="e in mesh.feed" :key="e.id" class="flex items-start gap-2.5">
                <span class="mt-0.5 h-2 w-2 shrink-0 rounded-full" :class="feedDot[e.kind] ?? 'bg-slate-400'"></span>
                <div class="min-w-0 flex-1">
                  <p class="text-xs leading-relaxed text-slate-300">{{ e.message }}</p>
                  <p class="text-[10px] text-slate-600">{{ fmtTime(e.time) }}</p>
                </div>
              </div>
            </div>
          </div>
