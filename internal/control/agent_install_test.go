...
func TestAgentInstallerAndBinaryDownload(t *testing.T) {
	...
	for _, required := range []string{"--server", "--token", "wiremesh-agent.service", "/agent/download"} {
...
func TestAgentHeartbeatUpdatesRealNode(t *testing.T) {
	...
	body := strings.NewReader(`{"hostname":"edge-01","os":"linux/amd64","agent_version":"0.2.0","labels":{"env":"prod"},"interfaces":"auto"}`)
	...
	if updated.LastSeen.IsZero() || updated.OS != "linux/amd64" || updated.AgentVersion != "0.2.0" || updated.Labels["env"] != "prod" {
